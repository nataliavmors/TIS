...
class Films_Data(object):
    ...
    # Возвращает список словарей с информацией по персонам с их id, name, birth, и death
        sql = text("SELECT person.person_name as name FROM person WHERE person.id=" + str(person_id) +" ; ")
    ...
    def get_person_birth(self,person_id):
    # Возвращает список словарей с информацией по персонам с их id, name, birth, и death
        sql = text("SELECT person.birth_date as birth FROM person WHERE person.id=" + str(person_id) +" ; ")
        sql_result = self._engine.execute(sql)
        ret = []
        for record in sql_result:
            dictionary = dict(record)
    ...
    def get_person_death(self,person_id):
    # Возвращает список словарей с информацией по персонам с их id, name, birth, и death
        sql = text("SELECT death.death_date AS death FROM person LEFT JOIN death ON person.id = death.dead_person_id WHERE person.id=" + str(person_id) +" ; ")
        sql_result = self._engine.execute(sql)
        ret = []
        for record in sql_result:
            dictionary = dict(record)
        return dictionary["death"]
    ...
    # Возвращает список словарей с информацией по фильмам с их id, name, release
        sql = text("SELECT film.film_name AS Name FROM film WHERE film.id=" + str(film_id) +";")
        sql_result = self._engine.execute(sql)
        ret = []
        for record in sql_result:
            dictionary = dict(record)
    ...
    def get_film_release(self,film_id):
    # Возвращает список словарей с информацией по фильмам с их id, name, release
        sql = text("SELECT film.release_date AS release FROM film WHERE film.id=" + str(film_id) +";")
        sql_result = self._engine.execute(sql)
        ret = []
        for record in sql_result:
            dictionary = dict(record)
    ...
    def get_person_roles(self,person_id):
        ...
        sql=text("SELECT film.id as film_id , roles.role_name as role, film.film_name as film_name FROM film JOIN film_person_role ON film_person_role.film_id=film.id JOIN roles ON film_person_role.role_id=roles.id  WHERE film_person_role.person_id = " + str(person_id) + ";")
        sql_result = self._engine.execute(sql)
        ret=[]
        for record in sql_result:
    ...
    def get_film_persons(self,film_id):
        sql=text("SELECT person.id as person_id, roles.role_name as role, person.person_name as name FROM person JOIN film_person_role ON film_person_role.person_id=person.id JOIN roles ON film_person_role.role_id=roles.id  WHERE film_person_role.film_id = " + str(film_id) + ";")
        sql_result = self._engine.execute(sql)
        ret=[]
        for record in sql_result:
            ret.append(dict(record))
