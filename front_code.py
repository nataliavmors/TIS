...
#"""Обрабатывает запрос к странице конретного автора http://mipt-space-tis.ru:5013/person """
    ...
    birth = bd.get_person_birth(person_id)
    death=bd.get_person_death(person_id)
    countries = bd.get_person_countries(person_id)
    job_list = bd.get_person_roles(person_id)
    ret = render_template("person.html", person_name = person_name, birth=birth, death=death , countries = countries, job_list = job_list)
    return ret
...
def film(film_id=None):
    ...
    release=bd.get_film_release(film_id)
    genres = bd.get_film_genres(film_id)
    countries = bd.get_film_countries(film_id)
    person_list = bd.get_film_persons(film_id)
    ret = render_template("film.html", release = release, countries = countries, person_list = person_list, genres = genres, film_name=film_name)
    return ret
